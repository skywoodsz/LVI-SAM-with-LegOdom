...
public:
    ...
    IMUPreintegration()
    {
        ...
        subOdometry = nh.subscribe<nav_msgs::Odometry>("/odom/leg", 5, &IMUPreintegration::odometryHandler, this, ros::TransportHints().tcpNoDelay());
        // skywoodsz
        subLegOdom            = nh.subscribe<nav_msgs::Odometry>      ("/odom/leg",                                50, &IMUPreintegration::LegOdomHandler, this, ros::TransportHints().tcpNoDelay());
    ...
    void imuHandler(const sensor_msgs::Imu::ConstPtr& imuMsg)
    {
        ...
        //     double alpha = 0.8;
        //     tf_odom.pose.pose.position.x = alpha * thisLegOdom.pose.pose.position.x * (1 - alpha) * odometry.pose.pose.position.x;
        //     tf_odom.pose.pose.position.y = alpha * thisLegOdom.pose.pose.position.y * (1 - alpha) * odometry.pose.pose.position.y;
        //     tf_odom.pose.pose.position.z = alpha * thisLegOdom.pose.pose.position.z * (1 - alpha) * odometry.pose.pose.position.z;            
        //     tf_odom.pose.pose.orientation = odometry.pose.pose.orientation;

        //     tf::Transform tCur;
        //     tf::poseMsgToTF(tf_odom.pose.pose, tCur);
        //     tf::StampedTransform odom_2_baselink = tf::StampedTransform(tCur, thisImu.header.stamp, "odom", "base_link");
        //     tfOdom2BaseLink.sendTransform(odom_2_baselink);
