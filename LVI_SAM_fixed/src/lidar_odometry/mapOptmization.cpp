...
public:
    ...
    std::deque<nav_msgs::Odometry> legOdomQueue;
    ...
    mapOptimization()
    {
        ...
        // skywoodsz
        subLegOdom            = nh.subscribe<nav_msgs::Odometry>      ("/odom/leg",                                50, &mapOptimization::LegOdomHandler, this, ros::TransportHints().tcpNoDelay());
    ...
    void laserCloudInfoHandler(const lvi_sam::cloud_infoConstPtr& msgIn)
    {
        ...
        if (timeLaserInfoCur - timeLastProcessing >= mappingProcessInterval) {
            ...
            // skywoodsz
            nav_msgs::Odometry thisLegOdom =  legOdomQueue.back();
            Eigen::Quaterniond q = Eigen::Quaterniond(thisLegOdom.pose.pose.orientation.w,
                                            thisLegOdom.pose.pose.orientation.x,
                                            thisLegOdom.pose.pose.orientation.y,
                                            thisLegOdom.pose.pose.orientation.z);
            ...
            transformTobeMapped[3] = thisLegOdom.pose.pose.position.x;
            transformTobeMapped[4] = thisLegOdom.pose.pose.position.y;
            transformTobeMapped[5] = thisLegOdom.pose.pose.position.z;
     ...
     * 
     */
    void LegOdomHandler(const nav_msgs::Odometry::ConstPtr& legOdomMsg)
    {
        std::lock_guard<std::mutex> lock(mtx);
        legOdomQueue.push_back(*legOdomMsg);
    ...
    void scan2MapOptimization()
    {
        ...
        if (laserCloudCornerLastDSNum > edgeFeatureMinValidNum && laserCloudSurfLastDSNum > surfFeatureMinValidNum)
        {
            ...
            // else
            // {
            //     ROS_WARN("Lidar Degenerate!");
            //     if(!legOdomQueue.empty())
            //     {
            //         nav_msgs::Odometry thisLegOdom = legOdomQueue.back();
            //         // tf::Transform tCur;
            //         // tf::poseMsgToTF(thisLegOdom.pose.pose, tCur);
            //         // tf::StampedTransform odom_2_baselink = tf::StampedTransform(tCur, thisLegOdom.header.stamp, "odom", "base_link");
     ...
     * 
     */
    void addLegOdomFactor()
    {
        ROS_WARN("Get addLegOdomFactor!");
        std::cout<<"\033[1;32m---->legOdomQueue.size(): \033[0m"<<legOdomQueue.size()<<std::endl; // 11
        std::cout<<"\033[1;32m---->cloudKeyPoses3D->points.size(): \033[0m"<<cloudKeyPoses3D->points.size()<<std::endl; // 11

        if(legOdomQueue.size() < 3)
            return ; 

        if (cloudKeyPoses3D->points.empty())
            return;

        ROS_WARN("Get addLegOdomFactor !2");

        while(legOdomQueue.size() > 3)
        {
            if (legOdomQueue.front().header.stamp.toSec() < timeLaserInfoCur - 0.1)
            {
                // message too old
                legOdomQueue.pop_front();
                continue;
            }
            else if (legOdomQueue.back().header.stamp.toSec() > timeLaserInfoCur + 0.1)
            {
                // message too new
                legOdomQueue.pop_back();
                continue;
            }
            else{
                break;
            ...
            // ROS_WARN("Get addLegOdomFactor!3");

            nav_msgs::Odometry lastLegOdom = legOdomQueue.front();
            nav_msgs::Odometry thisLegOdom = legOdomQueue.back();

            Eigen::Quaterniond last_q, this_q;
            last_q = Eigen::Quaterniond(lastLegOdom.pose.pose.orientation.w, 
                                    lastLegOdom.pose.pose.orientation.x,
                                    lastLegOdom.pose.pose.orientation.y,
                                    lastLegOdom.pose.pose.orientation.z);

            this_q = Eigen::Quaterniond(thisLegOdom.pose.pose.orientation.w, 
                                    thisLegOdom.pose.pose.orientation.x,
                                    thisLegOdom.pose.pose.orientation.y,
                                    thisLegOdom.pose.pose.orientation.z);

            Eigen::Matrix3d last_rot, this_rot;
            last_rot = last_q.toRotationMatrix();
            this_rot = this_q.toRotationMatrix();

            gtsam::Pose3 poseFrom = gtsam::Pose3(
                                            gtsam::Rot3(last_rot.col(0), last_rot.col(1), last_rot.col(2)), 
                                            gtsam::Point3(lastLegOdom.pose.pose.position.x,
                                                lastLegOdom.pose.pose.position.y,
                                                lastLegOdom.pose.pose.position.z)
            );

            gtsam::Pose3 poseTo = gtsam::Pose3(
                                            gtsam::Rot3(this_rot.col(0), this_rot.col(1), this_rot.col(2)), 
                                            gtsam::Point3(thisLegOdom.pose.pose.position.x,
                                                thisLegOdom.pose.pose.position.y,
                                                thisLegOdom.pose.pose.position.z)
            );

            noiseModel::Diagonal::shared_ptr odometryNoise = noiseModel::Diagonal::Variances((Vector(6) << 1, 1, 1, 1, 1, 1).finished());
            gtSAMgraph.add(BetweenFactor<Pose3>(cloudKeyPoses3D->size()-1, cloudKeyPoses3D->size(), poseFrom.between(poseTo), odometryNoise));                
    ...
    void saveKeyFramesAndFactor()
    {
        ...
        // {
            ...
            temp = gtsam::Pose3(gtsam::Rot3::RzRyRx(double(eulerAngle[2]), 
                                        double(eulerAngle[1]),
                                        double(eulerAngle[0])),
                                  gtsam::Point3(double(thisLegOdom.pose.pose.position.x), 
                                        double(thisLegOdom.pose.pose.position.y),  
                                        double(thisLegOdom.pose.pose.position.z)));
    ...
    void publishOdometry()
    {
        ...
        // {
            nav_msgs::Odometry thisLegOdom =  legOdomQueue.back();
            laserOdometryROS.pose = thisLegOdom.pose;
        ...
        // t_odom_to_lidar.setOrigin(tf::Vector3( laserOdometryROS.pose.pose.position.x, 
        //                     laserOdometryROS.pose.pose.position.y,
        //                     laserOdometryROS.pose.pose.position.z));

        // t_odom_to_lidar.setRotation(tf::Quaternion(laserOdometryROS.pose.pose.orientation.x,
        //                 laserOdometryROS.pose.pose.orientation.y,
        //                 laserOdometryROS.pose.pose.orientation.z,
        //                 laserOdometryROS.pose.pose.orientation.w
        //                 ));                  

        // br.sendTransform(tf::StampedTransform(t_odom_to_lidar, laserOdometryROS.header.stamp,  "odom", "lidar_link"));

        // tf::poseMsgToTF(laserOdometryROS.pose.pose, t_odom_to_lidar);
        ...
        // tf::StampedTransform odom_2_baselink = tf::StampedTransform(tCur, timeLaserInfoStamp, "odom", "lidar_link");
        // br.sendTransform(odom_2_baselink);
